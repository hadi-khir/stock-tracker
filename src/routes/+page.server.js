...
export async function load({ fetch }) {
  ...
  let slugs = "";
  screenerResultsData.data.forEach((/** @type {any} */ stock) => {
    slugs += stock.attributes.name + "%2C";
  });

  const metricsReq = await fetch(
    `https://seekingalpha.com/api/v3/metrics?filter[fields]=quant_rating%2Cauthors_rating%2Csell_side_rating%2Cmarketcap_display%2Cdividend_yield&filter[slugs]=${slugs}&minified=false`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  const combinedStockData = screenerResultsData.data.map((
    /** @type {any} */ stock,
    /** @type {number} */ idx
  ) => {
    ...
    let marketCapInBillionsString =
      parseFloat(marketCap) > 999999999
    ...
    const wallStreetRating = metricsData.data.find((
      /** @type {any} */ metric
    ) => {
      return (
    ...
    const saAnalystRating = metricsData.data.find((
      /** @type {any} */ metric
    ) => {
    ...
    // get profitability
    const profitability = gradeMap.get(
      tickerGradeData.data.find((/** @type {any} */ metric) => {
        return (
          metric.id ===
    ...
    // get eps revision
    const epsRevision = gradeMap.get(
      tickerGradeData.data.find((/** @type {any} */ metric) => {
        return (
          metric.id === `${stockId},${ratingsMap.get("epsRevision")},main_quant`
        );
      })?.attributes?.grade
    );
